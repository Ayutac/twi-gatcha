...
public interface Attacks {
    ...
    Attack WEAK_PUNCH = new Attack(
            ...
            1, 1, 1,
            List.of(new ApplicableEffect(EffectType.DAMAGE_BLUNT, 1, 0, 0, 0, null, null)));

    Attack MEDIUM_PUNCH = new Attack(
            "Medium Punch",
            "Punch like an average person.",
            1, 1, 1,
            List.of(new ApplicableEffect(EffectType.DAMAGE_BLUNT, 2, 0, 0, 0, null, null)));

    Attack MINOTAUR_PUNCH = new Attack(
            "[Minotaur Punch]",
            "Punch like a Minotaur.",
            1, 1, 1,
            List.of(new ApplicableEffect(EffectType.DAMAGE_BLUNT, 3, 0, 0, 0, null, null)));

    Attack UNERRING_KNIFE_THROW = new Attack(
            "Unerring Knife Throw",
            "[Unerring Throw] with a knife.",
            2, 4, 2,
            List.of(new ApplicableEffect(EffectType.DAMAGE_SLASH, 2, 0, 0, 0, null, null)));

    Attack PASTA = new Attack(
            "Erin's lunch special",
            "Pasta with a glass of Blue Fruit Juice.",
            1, 1, 2,
            List.of(new ApplicableEffect(EffectType.HEALING, 2, 0, 0, 0, null, null)));

    Attack QUICK_SLASH = new Attack(
            "Quick Slash",
            "Quick slash with a sword or something similar.",
            1, 1, 2,
            List.of(new ApplicableEffect(EffectType.DAMAGE_SLASH, 3, 0, 0, 0, null, null)));

    Attack ARROW = new Attack(
            "Shoot Arrow",
            "Shoot a regular arrow at your target.",
            2, 6, 3,
            List.of(new ApplicableEffect(EffectType.DAMAGE_PIERCE, 3, 0, 0, 0, null, null)));
    ...
    Attack BONE_FRACTURE = new Attack(
            ...
            1, 1, 5,
            List.of(new ApplicableEffect(EffectType.DAMAGE_DEATH, 5, 0, 0, 0, null, null)));
    ...
    Attack DEATH_BOLT = new Attack(
            ...
            2, 4, 1,
            List.of(new ApplicableEffect(EffectType.DAMAGE_DEATH, 10, 0, 0, 0, null, null)));

    Attack ICE_SHARD = new Attack(
            "[Ice Shard]",
            "Shoot a frozen bit of water at your target.",
            2, 4, 1,
            List.of(new ApplicableEffect(EffectType.DAMAGE_SLASH, 1, 0, 0, 0, null, null),
                    new ApplicableEffect(EffectType.DAMAGE_FROST, 1, 0, 0, 0, null, null)));

    Attack FROST_ARMOR = new Attack(
            "Frost Armor",
            "Cover yourself in a layer of ice.",
            0, 0, 5,
            List.of(new ApplicableEffect(EffectType.BUFF_DEFENSE, 5, 3, 0, 0, null, null)));

    Attack FROZEN_WIND = new Attack(
            "Frozen Wind",
            "Cover yourself in a layer of ice.",
            2, 4, 5,
            List.of(new ApplicableEffect(EffectType.DAMAGE_FROST, 2, 0, 1, 0, null, null),
                    new ApplicableEffect(EffectType.DEBUFF_SPEED, 2, 3, 1, 0, null, null)));

    Attack QUICK_MOVEMENT = new Attack(
            "Quick Movement",
            "Be faster for a while.",
            0, 0, 5,
            List.of(new ApplicableEffect(EffectType.BUFF_SPEED, 7, 3, 0, 0, null, null)));

    Attack SIDE_STEP = new Attack(
            "Side Step",
            "Dodge the next enemy attack.",
            0, 0, 5,
            List.of(new ApplicableEffect(EffectType.INVULNERABILITY, 0, 3, 0, 0, null, null)));

    Attack IGNORE_PAIN = new Attack(
            "Ignore Pain",
            "Just ignore it!",
            0, 0, 5,
            List.of(new ApplicableEffect(EffectType.BUFF_HEALTH, 15, 3, 0, 0, null, null)));

    Attack KEEN_EDGE = new Attack(
            "Keen Edge",
            "Use the sharp end of your weapon.",
            0, 0, 5,
            List.of(new ApplicableEffect(EffectType.BUFF_ATTACK, 5, 3, 0, 0, null, null)));

    Attack INVISIBILITY = new Attack(
            "[Invisibility]",
            "Become invisible.",
            0, 0, 5,
            List.of(new ApplicableEffect(EffectType.INVISIBILITY, 0, 3, 0, 0, null, null)));
    ...
    Attack UNDEAD_BITE = new Attack(
            ...
            "A foul bite.",
            1, 1, 3,
            List.of(new ApplicableEffect(EffectType.DAMAGE_PIERCE, 4, 0, 0, 0, null, null)));
